#!/usr/bin/env node
/**
 * Sync data registrasi HUT Kota Makassar dari API workflow → tabel MySQL
 * `tb_event_hut`. Ini sumber data utama dashboard HUT Kota Makassar.
...
//   "*/10 * * * * cd /path/to/superapps-nextjs && node scripts/warmup-hut.mjs >> /var/log/hut-warmup.log 2>&1"
...
const USER = process.env.HUT_API_USER || "support@lontaraplus.makassarkota.go.id";
...
async function fetchPage(page) {
  const url = `${BASE}/event_registrations?page=${page}&limit=${LIMIT}&event_type=hut`;
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      ...
      console.log(`[warmup-hut] page ${page} percobaan ${attempt} gagal (${err.message}) — tunggu ${wait / 1000}s`);
...
async function upsertBatch(conn, rows) {
  ...
  const update = cols
    ...
    .map((c) => `\`${c}\` = VALUES(\`${c}\`)`)
    .join(", ");
  const sql = `INSERT INTO tb_event_hut (\`${cols.join("`,`")}\`)
    VALUES ${placeholders}
...
async function main() {
  const t0 = Date.now();
  console.log(`[warmup-hut] mulai sync HUT dari ${BASE} ...`);
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    console.log(`[warmup-hut] page ${page} OK (+${data.length}, total ${allRows.length})`);
    if (data.length < LIMIT) break;
  }

  if (allRows.length === 0) throw new Error("Tidak ada data HUT");

  console.log(`[warmup-hut] total ${allRows.length} baris → upsert ke tb_event_hut ...`);
  ...
  const [cnt] = await conn.query("SELECT COUNT(*) n FROM tb_event_hut");
  console.log(
    `[warmup-hut] selesai: ${allRows.length} baris diproses (affected ${affected}), ` +
...
main().catch((err) => {
  console.error("[warmup-hut] GAGAL:", err.message);
  process.exit(1);
});